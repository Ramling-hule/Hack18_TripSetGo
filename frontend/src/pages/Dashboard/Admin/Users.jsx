  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.875rem', fontWeight: 800, marginBottom: '0.25rem' }}>Manage <span className="bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent">Users</span></h1>
          <p style={{ color: 'var(--color-text-secondary)' }}>Review, update roles, toggle account statuses, and soft-delete users</p>
        </div>
        <button className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold cursor-pointer border border-border bg-transparent text-text-primary hover:border-primary/45 hover:bg-white/5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_0_40px_rgba(14,165,233,0.25)]" onClick={handleExportCSV} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <table className="w-full border-collapse" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-secondary)' }}>
                <th className="text-[0.75rem] uppercase tracking-wider text-text-muted font-semibold" style={{ padding: '1rem' }}>User</th>
                <th className="text-[0.75rem] uppercase tracking-wider text-text-muted font-semibold" style={{ padding: '1rem' }}>Role</th>
                <th className="text-[0.75rem] uppercase tracking-wider text-text-muted font-semibold" style={{ padding: '1rem' }}>Status</th>
                <th className="text-[0.75rem] uppercase tracking-wider text-text-muted font-semibold" style={{ padding: '1rem' }}>Plan</th>
                <th className="text-[0.75rem] uppercase tracking-wider text-text-muted font-semibold" style={{ padding: '1rem' }}>Joined Date</th>
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user._id} className="transition-colors duration-150 hover:bg-white/[0.02]" style={{ borderBottom: '1px solid var(--color-border)' }}>
                  ...
                  <td style={{ padding: '1rem' }}>
                    <Badge label={user.role} variant={user.role === 'admin' ? 'primary' : 'secondary'} className="capitalize" />
                  </td>
                  <td style={{ padding: '1rem' }}>
                    <Badge label={user.status} variant={user.status === 'active' ? 'success' : user.status === 'suspended' ? 'warning' : 'danger'} className="capitalize" />
                  </td>
                  <td style={{ padding: '1rem' }}>
                    <Badge label={user.plan} variant={user.plan === 'pro' ? 'pro' : 'secondary'} className="uppercase" />
                  </td>
                  ...
                  <td style={{ padding: '1rem', textAlign: 'right' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                      {/* Toggle status */}
                      <button
                        className="inline-flex items-center justify-center p-2 rounded-lg bg-transparent border border-border text-text-secondary cursor-pointer transition-all duration-150 hover:bg-white/5 hover:border-border-hover disabled:opacity-40 disabled:cursor-not-allowed"
