...
export default function Analytics() {
  ...
  const customTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-bg-card/75 backdrop-blur-[20px] border border-border shadow-[inset_0_0_20px_rgba(255,255,255,0.02)]" style={{ padding: '0.75rem 1rem', borderRadius: 'var(--radius-md)', fontSize: '0.85rem' }}>
  ...
  if (loading && trips.length === 0) {
    return <div className="animate-fadeIn"><Loader text="Loading your analytics..." /></div>
  }

  return (
    <div className="animate-fadeIn">
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.875rem', fontWeight: 800, marginBottom: '0.25rem' }}>Travel <span className="bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent">Analytics</span></h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
        {[
          ...
          <div key={s.label} className="bg-bg-card border border-border rounded-xl p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]" style={{ textAlign: 'center', padding: '1.5rem 1rem' }}>
            <p style={{ fontSize: '1.75rem', fontWeight: 800 }} className="bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent">{s.value}</p>
      ...
      {trips.length === 0 ? (
        <div className="bg-bg-card border border-border rounded-xl p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]" style={{ textAlign: 'center', padding: '4rem', color: 'var(--color-text-muted)' }}>
